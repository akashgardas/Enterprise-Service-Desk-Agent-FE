...
const AnalyticsDashboard = () => {
  ...
  const handleExport = (format) => {
    alert(`Exporting as ${format}... In a real app, this would trigger a file download.`);
  };
  ...
  return (
    <div className="space-y-10 animate-fade-in pb-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h1 className="text-3xl font-extrabold text-text-primary tracking-tight">Analytics & Insights</h1>
          <p className="text-text-secondary mt-1 text-lg">Detailed performance metrics and service trends</p>
        </div>
        <div className="flex items-center gap-4 w-full md:w-auto">
          <div className="relative flex-grow md:flex-grow-0">
            <HiOutlineCalendarDays className="absolute left-3 top-1/2 -translate-y-1/2 text-text-secondary w-5 h-5" />
            <select 
              value={timeRange}
              onChange={(e) => setTimeRange(e.target.value)}
              className="input-field pl-10 py-2.5 text-sm font-semibold pr-10 appearance-none bg-white"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="card p-8">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-xl font-bold text-text-primary">Monthly Ticket Volume</h3>
            <span className="text-xs font-bold text-text-secondary uppercase tracking-widest">Trend Analysis</span>
          </div>
          <div className="h-80">
        ...
        <div className="card p-8">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-xl font-bold text-text-primary">Category Distribution</h3>
            <span className="text-xs font-bold text-text-secondary uppercase tracking-widest">Volume by Team</span>
          </div>
        ...
        <div className="card p-8">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-xl font-bold text-text-primary">Priority Breakdown</h3>
            <span className="text-xs font-bold text-text-secondary uppercase tracking-widest">Urgency Levels</span>
          </div>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="card p-8">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-xl font-bold text-text-primary">SLA Compliance</h3>
            <span className="text-xs font-bold text-text-secondary uppercase tracking-widest">Target Monitoring</span>
          </div>
          <div className="space-y-10 py-4">
            <div className="flex justify-between items-center">
              <div>
                <p className="text-sm text-text-secondary font-bold uppercase tracking-widest mb-1">Overall Compliance</p>
                <p className="text-5xl font-extrabold text-text-primary tracking-tighter">{data?.slaCompliance.percentage}%</p>
              </div>
              <div className={`px-4 py-2 rounded-2xl text-xs font-bold ${data?.slaCompliance.trend > 0 ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                {data?.slaCompliance.trend > 0 ? 'Γåæ' : 'Γåô'} {Math.abs(data?.slaCompliance.trend)}% vs last month
              </div>
            </div>
...
const SLAProgressBar = ({ label, percentage, color }) => (
  <div className="space-y-2">
    <div className="flex justify-between text-sm">
      <span className="font-bold text-text-secondary uppercase tracking-widest text-[10px]">{label}</span>
      <span className="font-extrabold text-text-primary">{percentage}%</span>
    </div>
    <div className="w-full bg-bg-secondary rounded-full h-3 overflow-hidden border border-border-color/50">
